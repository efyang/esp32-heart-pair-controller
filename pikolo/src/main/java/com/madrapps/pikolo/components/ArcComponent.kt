...
internal abstract class ArcComponent(metrics: Metrics, paints: Paints) : ColorComponent(metrics, paints) {
    ...
    override operator fun contains(point: PointF): Boolean {
        val touchRadius = (indicatorRadius + indicatorStrokeWidth) * 1.5
        val x = point.x - metrics.centerX
        val y = point.y - metrics.centerY
        val r = Math.hypot(x.toDouble(), y.toDouble())

        var potentialAngle = Math.toDegrees(Math.acos(x / r))
        if (y < 0) {
            potentialAngle = 360 - potentialAngle
        }

        var angleInRange = true
        if (arcStartAngle < arcEndAngle) {
            angleInRange = potentialAngle in (arcStartAngle..arcEndAngle)
        } else {
            angleInRange = (potentialAngle in (arcStartAngle..360f)) || (potentialAngle in (0f..arcEndAngle))
        }
        return (r in (radius - touchRadius)..(radius + touchRadius)) && angleInRange
    }
